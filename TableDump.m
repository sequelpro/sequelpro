...
- (void)savePanelDidEnd:(NSSavePanel *)sheet returnCode:(int)returnCode contextInfo:(NSString *)contextInfo
{
	...
	// Export the current "browse" view to a file in CSV format
		...
		success = [self writeCsvForArray:[tableContentInstance currentResult] toFileHandle:fileHandle
							useFirstLine:[exportFieldNamesSwitch state]
	...
	// Export the current "browse" view to a file in XML format
		...
		success = [self writeXmlForArray:[tableContentInstance currentResult] toFileHandle:fileHandle
	...
	// Export the current custom query result set to a file in CSV format
		...
		success = [self writeCsvForArray:[customQueryInstance currentResult] toFileHandle:fileHandle
							useFirstLine:[exportFieldNamesSwitch state]
							terminatedBy:[exportFieldsTerminatedField stringValue]
							enclosedBy:[exportFieldsEnclosedField stringValue]
							escapedBy:[exportFieldsEscapedField stringValue]
	...
	// Export the current custom query result set to a file in XML format
		...
		success = [self writeXmlForArray:[customQueryInstance currentResult] toFileHandle:fileHandle
							tableName:@"custom"
							withHeader:YES
							silently:NO];
			...
			if (rowCount) {
				...
				for ( j = 0 ; j < rowCount ; j++ ) {
					...
					for ( t = 0 ; t < [theRow count] ; t++ ) {
						...
						// Add data types directly as escaped data
 ...
 */
- (BOOL)writeCsvForArray:(NSArray *)array toFileHandle:(NSFileHandle *)fileHandle useFirstLine:(BOOL)firstLine terminatedBy:(NSString *)fieldSeparatorString
	enclosedBy:(NSString *)enclosingString escapedBy:(NSString *)escapeString lineEnds:(NSString *)lineEndString silently:(BOOL)silently;
{
	...
	for ( i = (firstLine ? 0 : 1) ; i < [array count] ; i++ ) {
		...
		for ( j = 0 ; j < [csvRow count] ; j++ ) {
			...
			// For NULL values no cell needs to be written.
 ...
 */
- (BOOL)writeXmlForArray:(NSArray *)array toFileHandle:(NSFileHandle *)fileHandle tableName:(NSString *)table withHeader:(BOOL)header silently:(BOOL)silently
{
	NSStringEncoding tableEncoding = [CMMCPConnection encodingForMySQLEncoding:[[tableDocumentInstance encoding] cString]];
	...
	for ( i = 1 ; i < [array count] ; i++ ) {
		...
		for ( j = 0 ; j < [xmlRow count] ; j++ ) {
			...
			[xmlString appendString:[self htmlEscapeString:[[xmlRow objectAtIndex:j] description]]];
...
- (BOOL)exportTables:(NSArray *)selectedTables toFileHandle:(NSFileHandle *)fileHandle usingFormat:(NSString *)type
{
	...
	for ( i = 0 ; i < [selectedTables count] ; i++ ) {
		...
		// Clear any data from previous tables and add the table's field names as the first row
		[tableArray removeAllObjects];
		[tableArray addObject:fieldNames];

		// Step through the result set rows and add them to the array
		for ( j = 0 ; j < rowCount ; j++ ) {
			[queryResult dataSeek:j];
			theRow = [queryResult fetchRowAsArray];
		...
		if ( [type isEqualToString:@"csv"] ) {
			[self writeCsvForArray:tableArray toFileHandle:fileHandle
			...
			[self writeXmlForArray:tableArray toFileHandle:fileHandle tableName:tableName withHeader:NO silently:YES];
		}
