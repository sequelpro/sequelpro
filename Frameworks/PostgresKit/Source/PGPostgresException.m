...
@implementation PGPostgresException
...
+ (void)raise:(NSString *)name connection:(void *)connection 
{
	...
	[[[[PGPostgresException alloc] initWithName:name reason:[NSString stringWithUTF8String:errorMessage] userInfo:nil] autorelease] raise];
}
...
+ (void)raise:(NSString *)name reason:(NSString *)reason, ...
{
	...
	[[[[PGPostgresException alloc] initWithName:name reason:reasonMessage userInfo:nil] autorelease] raise];
